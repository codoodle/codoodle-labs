...
export interface GridColumnsHeaderEventMap<
  TSource extends GridColumnsHeader = GridColumnsHeader,
> {
  /**
...
export class GridColumnsHeader extends SupportPropertyChange {
  protected _rowsFactory: () => GridRow[];
  ...
  constructor(
    ...
    this._rowsFactory =
      rowsFactory ??
      (() => {
        return [new GridRow(30)];
      });
    this._isCollapsed = isCollapsed;
    this._merges = merges;
  }

  on<K extends keyof GridColumnsHeaderEventMap<this>>(
    name: K,
    handler: (this: this, args: GridColumnsHeaderEventMap<this>[K]) => void,
  ): () => void;
  on(
    name: string,
  ...
  once<K extends keyof GridColumnsHeaderEventMap<this>>(
    name: K,
    handler: (this: this, args: GridColumnsHeaderEventMap<this>[K]) => void,
  ): () => void;
  once(
  ...
  off<K extends keyof GridColumnsHeaderEventMap<this>>(
    name: K,
    handler?: (this: this, args: GridColumnsHeaderEventMap<this>[K]) => void,
  ): void;
  off(
  ...
  protected emit<K extends keyof GridColumnsHeaderEventMap<this>>(
    args: GridColumnsHeaderEventMap<this>[K],
  ): boolean;
